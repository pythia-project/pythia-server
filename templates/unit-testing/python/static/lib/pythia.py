...
FILE_PERM = stat.S_IRWXU | stat.S_IRWXG | stat.S_IROTH
...
def generateTestData(dest, filename, config):
    ...
    with open(filedest, 'w', encoding='utf-8') as file:
        ...
        if 'predefined' in config:
            for case in config['predefined']:
                writer.writerow(ast.literal_eval(case['data']))
...
class FeedbackSuite:
    ...
    def __init__(self, inputfile, resultfile, config, spec):
        self.inputfile = inputfile
